...
class RepositoryAnalyzer:
    ...
    def _generate_repo_hash(self, repo_url: str, ref: str) -> str:
        """Generate a hash for caching based on repo URL and ref."""
    ...
    def _is_important_file(self, path: Path) -> Tuple[bool, int]:
        """
        ...
        # Priority 2: Important project files
        important_root_files = {'package.json', 'requirements.txt', 'pyproject.toml',
                               'cargo.toml', 'go.mod', 'pom.xml', 'build.gradle',
                               'composer.json', 'gemfile'}
        if len(parts) == 1 and filename in important_root_files:
            return True, 2
    ...
    def _load_repository_context(self, repo_url: str, ref: str = "main") -> Tuple[List[Dict], RepositoryMetadata]:
        """
        ...
        repo_hash = self._generate_repo_hash(repo_url, ref)
        ...
        try:
            ...
            for path in repo_path.rglob("*"):
                if path.is_file():
                    is_important, priority = self._is_important_file(path)
                    if is_important:
            ...
            logger.info(f"Loaded {len(context_chunks)}/{len(all_files)} files for analysis")
    ...
    def _create_analysis_prompt(self, context_chunks: List[Dict], metadata: RepositoryMetadata) -> str:
        ...
        prompt = f"""
You are an expert software architect analyzing a codebase. Please provide a comprehensive analysis of this repository.
...
{{
    "summary": "Brief 2-3 sentence summary of what this codebase does",
...
"""
    ...
    def analyze_repository(self, repo_url: str, ref: str = "main", force_refresh: bool = False) -> AnalysisResult:
        """
        ...
        if not force_refresh:
            ...
            if existing:
                logger.info(f"Using cached analysis for {repo_url}#{ref}")
                return existing
        
        logger.info(f"Starting analysis of {repo_url}#{ref}")
        
        try:
            # Load repository context
            context_chunks, metadata = self._load_repository_context(repo_url, ref)
    ...
    def _store_analysis_result(self, analysis: AnalysisResult):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                (repo_hash, repo_url, ref_branch, analysis_timestamp, metadata, analysis_result, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    def get_analysis_history(self, repo_url: Optional[str] = None) -> List[Dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            if repo_url:
                cursor = conn.execute("""
                    SELECT repo_url, ref_branch, analysis_timestamp, created_at
    ...
    def export_analysis(self, repo_url: str, ref: str = "main") -> Optional[Dict]:

...
def analyze_command(args):
    ...
    try:
        ...
        print(f"Analysis Time: {result.repository_metadata.analysis_timestamp}")
...
def history_command(args):
    ...
    analyzer = RepositoryAnalyzer(db_path=args.db_path)
...
def export_command(args):
    ...
    result = analyzer.export_analysis(args.repo_url, args.ref)
    if not result:
        print(f"No analysis found for {args.repo_url}#{args.ref}")
        sys.exit(1)
...
def main():
    ...
    analyze_parser.add_argument('--ref', '-r', default='main', help='Branch or tag reference')
    analyze_parser.add_argument('--force', '-f', action='store_true', help='Force re-analysis')
    analyze_parser.add_argument('--max-files', type=int, default=25, help='Maximum files to analyze')
    analyze_parser.add_argument('--max-chars', type=int, default=6000, help='Maximum characters per file')
    analyze_parser.add_argument('--model', default='gpt-4o-mini', help='OpenAI model to use')
    analyze_parser.add_argument('--output', '-o', help='Save full results to JSON file')
    analyze_parser.set_defaults(func=analyze_command)
    
    # History command
    history_parser = subparsers.add_parser('history', help='Show analysis history')
    history_parser.add_argument('repo_url', nargs='?', help='Filter by repository URL')
    ...
    export_parser.add_argument('--ref', '-r', default='main', help='Branch or tag reference')
